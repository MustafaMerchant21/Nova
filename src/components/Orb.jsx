...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    vec3 adjustHue(vec3 color, float hueDeg) {
      ...
      float newI = yiq.y * cosH - yiq.z * sinH;
      float newQ = yiq.y * sinH + yiq.z * cosH;
      return yiq2rgb(vec3(yiq.x, newI, newQ));
    }
    ...
    float snoise3(vec3 p) {
      ...
      return dot(vec4(52.0), n);
    }
    
    vec4 extractAlpha(vec3 colorIn) {
      vec4 colorOut;
      float maxValue = max(max(colorIn.r, colorIn.g), max(colorIn.b, 1e-6));
      if (maxValue > 1.0) {
        colorOut.rgb = colorIn.rgb / maxValue;
        colorOut.a = maxValue;
      } else {
        colorOut.rgb = colorIn.rgb;
        colorOut.a = 1.0;
      }
      return colorOut;
    }
